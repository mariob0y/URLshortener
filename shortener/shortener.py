...
def create(user, link):
    ...
    try:
        ...
        enabled = p.enabled if p.enabled is not None \
            else getattr(settings, 'SHORTENER_ENABLED', True)
        max_urls = p.max_urls if p.max_urls is not None \
            else getattr(settings, 'SHORTENER_MAX_URLS', -1)
        max_concurrent = p.max_concurrent_urls if p.max_concurrent_urls \
            is not None else getattr(settings, 'SHORTENER_MAX_CONCURRENT', -1)
        lifespan = p.default_lifespan if p.default_lifespan \
            is not None else getattr(settings, 'SHORTENER_LIFESPAN', -1)
        max_uses = p.default_max_uses if p.default_max_uses \
            is not None else getattr(settings, 'SHORTENER_MAX_USES', -1)
    ...
    if lifespan != -1:
        ...
        expiry_date = timezone.make_aware(timezone.datetime.max,
    ...
    if max_concurrent != -1:
        if UrlMap.objects.filter(user=user,
                                 date_expired__gt=timezone.now()).count() >= \
                                                  max_concurrent:
    ...
    for tries in range(3):
        try:
            ...
            m = UrlMap(user=user,
                       full_url=link, short_url=short, max_count=max_uses,
