...
class UrlMapTestCase(TestCase):
    ...
    def test_invalid_link(self):
        ...
        with self.assertRaisesMessage(KeyError, 'invalid shortlink'):
            self.assertEqual(shortener.expand(
                'photosynthesis'), 'http://devget.net/')

    def test_shortener_enabled_setting(self):
        with self.settings(SHORTENER_ENABLED=False):
            with self.assertRaisesMessage(PermissionError,
                 'not authorized to create shortlinks'):
    ...
    def test_lifespan_setting(self):
        with self.settings(SHORTENER_LIFESPAN=4):   # 4 seconds
            ...
            with self.assertRaisesMessage(PermissionError,
                                          'shortlink expired'):
                self.assertEqual(shortener.expand(url),
                                 "http://blog.devget.net/")
...
class UrlProfileTestCase(TestCase):
    ...
    """
    def setUp(self):
        self.ronald = get_user_model().objects.create_user('ronald',
                                                           'ronald@ronald.com',
                                                           'ronaldpassword')
